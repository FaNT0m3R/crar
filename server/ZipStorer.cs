...
namespace System.IO.Compression
{
    ...
    public class ZipStorer : IDisposable
    {
        ...
        */
        ...
        /* DOS Date and time:
            MS-DOS date. The date is a packed value with the following format. Bits Description 
                0-4 Day of the month (1Ц31) 
                5-8 Month (1 = January, 2 = February, and so on) 
                9-15 Year offset from 1980 (add 1980 to get actual year) 
            MS-DOS time. The time is a packed value with the following format. Bits Description 
                0-4 Second divided by 2 
                5-10 Minute (0Ц59) 
                11-15 Hour (0Ц23 on a 24-hour clock) 
        */
        ...
        */
        ...
        //---------------------------
        public Stream ExtractStream(ZipFileEntry _zfe)
        {
        ...
        /// <summary>
        /// распаковывает данный
        /// </summary>
        /// <param name="_zfe"></param>
        /// <param name="inStream">поток созданный ExtractStream</param>
        /// <param name="_stream">поток дл€ записи распакованных данных</param>
        /// <param name="pos">позици€ чтени€. ¬ычисл€етс€ на основе результата этой функции</param>
        /// <param name="size">сколько читать</param>
        /// <returns>количестов прочитанных байт</returns>
        public int ExtractRead(ZipFileEntry _zfe, Stream inStream, Stream _stream, uint pos, int size)
        {
            byte[] buffer = new byte[16384];
            this.ZipFileStream.Seek(_zfe.FileOffset, SeekOrigin.Begin);
            uint bytesPending = _zfe.FileSize - pos;
            int sumRead = 0;

            while ((bytesPending > 0) && (sumRead < size))
            {
                int rsize = (int)Math.Min(buffer.Length, size);
                int bytesRead = inStream.Read(buffer, 0, (int)Math.Min(bytesPending, rsize));
                _stream.Write(buffer, 0, bytesRead);
                bytesPending -= (uint)bytesRead;
                sumRead += bytesRead;
            }
        ...
        public int ExtractReadInNull(ZipFileEntry _zfe, Stream inStream, uint pos, int size)
        {
            byte[] buffer = new byte[16384];
            this.ZipFileStream.Seek(_zfe.FileOffset, SeekOrigin.Begin);
            uint bytesPending = _zfe.FileSize - pos;
            ...
            while ((bytesPending >= 0x20) && (sumRead < size))
            {
                int bytesRead = inStream.Read(buffer, 0, (int)Math.Min(bytesPending, rsize));
                bytesPending -= (uint)bytesRead;
                sumRead += bytesRead;
        ...
        public ZipFileEntry AS_Create(Compression _method, string _filenameInZip, DateTime _modTime, string _comment)
        {
            if (Access == FileAccess.Read)
                throw new InvalidOperationException("Writing is not alowed");
        ...
        public Stream AS_GetStream(ZipFileEntry _zfe, int pos)
        {
            Stream outStream;

            long posStart = this.ZipFileStream.Position;
            long sourceStart = pos;//_source.Position;
        ...
        public uint AS_Write(ZipFileEntry _zfe, Stream outStream,Stream _source, uint size)
        {
            uint bytesRead;
            uint totalRead = 0;
            byte[] buffer = new byte[Math.Min(1048576,size)];
            
            ///////////////////
            do
            {
                bytesRead = (uint)_source.Read(buffer, 0, Math.Min(buffer.Length, (int)(size - totalRead)));
                totalRead += bytesRead;
                if (bytesRead > 0)
                {
        ...
        public void AS_Close(ZipFileEntry zfe,Stream outStream, uint totalRead)
        {
            ...
            zfe.CompressedSize = (uint)(this.ZipFileStream.Position);
